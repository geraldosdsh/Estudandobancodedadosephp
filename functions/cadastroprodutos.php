...
function cadastro(
    ...
    global $db;
    if (verifica_produto_exist($nome) < 1 ){
        $stmt = $db->prepare(
        ...
        if ($stmt->execute()) {
            echo '<script type="text/javascript">alert ("O produto foi cadastrado com sucesso") </script>'. '<div class="row mx-md-n5 mt-5">
        ...
        echo '<script type="text/javascript">alert ("Ops o produto ja foi cadastrado anteriormente")</script>';
       }
   

}

function verifica_produto_exist($nome){
    global $db;
    $stmt = $db->prepare('select count(id) from tb_cadastro_produto where nome = :nome');
    $stmt->bindParam(':nome', $nome);
    $stmt->execute();
