...
function get()
{
    ...
    $stmt = $db->prepare('select * from tb_emails_contato');
    $stmt->execute();
    $result = $stmt->fetchAll(PDO::FETCH_ASSOC);

    // Exibe os dados
    echo '<table class="table">';
    echo '<tr><th>ID</th><th>Nome</th><th>Email</th><th>Telefone</th><th>Status</th><th>Ações</th></tr>';
    foreach ($result as $row) {
        ...
        echo '<td>' . verifica_status($row['status']) . '</td>';
        echo '<td>' . verifica_disp_indsp($row['status']) . '<td>';
        echo '</tr>';
